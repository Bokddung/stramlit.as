...
def main():
    ...
    print(df.sort_values('petal_length',ascending=True))

    # 체크박스 : 둘중에 하나만 선택하게끔 만들때.(체크/해제)
    st.checkbox('헤드 5개 보기')
    # 체크하면, 헤드 5개 보여주고, 헤제하면 안보여주도록.
    st.checkbox('헤드 5개 보기') 
    st.dataframe(df.head())

    # 셀렉트 박스 : 여러개에서 한개만 고르게 하되 리스트가 많을때 사용한다.
    language = ['Python','c','Java','Go','PHP','Dart']
    my_choice = st.selectbox('좋아하는 언어 선택')
    if my_choice == language[0] or my_choice == language

    # 멀티셀렉트 : 여러개중에서, 여러개를 선택하고 선택하게 할때

    # 유저가 선택한 컬럼을 데이터프레임을 보여주되
    # 아무것도 선택안하면 아무것도 나오지 않게 하시오
    choice_list = st.multiselectdf('원하는 컬럼을 선택하세요',df.columns)
